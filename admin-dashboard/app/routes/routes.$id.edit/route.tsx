...
export default function EditRoutePage() {
  ...
  // Populate form when route data loads
  useEffect(() => {
    if (route) {
      reset({
        name: route.name,
  ...
  const updateMutation = useMutation({
    mutationFn: (data: RouteFormData) => routeService.updateRoute(id!, data),
    onSuccess: () => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-3 text-gray-600">Loading route data...</span>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Update route information</p>
        </div>
      </div>
